...
# - Downloads TESS light curves by TIC ID (or reads local CSV with time/flux)
# - Extracts features via fast BLS (coarse→fine)
...
def fast_bls_features(time, flux,
    ...
    if min_period is None:
        min_period = max(0.25, 2 * (np.median(np.diff(np.sort(t))) + 1e-6))
    if max_period is None:
        max_period = max(1.0, min(0.8 * baseline, 50.0))
    if min_period >= max_period:
        min_period, max_period = 0.5, 5.0

    dur_grid = np.linspace(0.5/24, 8/24, 24)
    ...
    depth = float(res2.depth[i2])
    snr = float(res2.snr[i2])
...
def build_model(name: str, seed: int, class_weight=None):
    ...
    if name == "xgb":
        ...
        # We'll handle the old/new API in train_xgb()
...
def main():
    ...
    ap.add_argument("--tic_ids", nargs="*", default=None, help="List of TIC IDs (TESS) to download")
    ap.add_argument("--csv_glob", default=None, help=r'Glob of local CSVs with time/flux, e.g. "C:\path\*.csv"')
    ...
    ap.add_argument("--coarse", type=int, default=1200, help="Coarse period grid")
    ap.add_argument("--fine", type=int, default=350, help="Fine period grid")
    ...
    if hasattr(model, "predict_proba"):
        ...
        # Canonical 3-class columns (order: CONFIRMED, CANDIDATE, FALSE POSITIVE)
        # Note: your internal encoding is 1,0,-1, so we’ll map by argmax into {1,0,-1}
        pred_idx = np.argmax(proba, axis=1)
        # Try to infer mapping if model trained with 3 classes in indices {0,1,2}
        # We'll build a vote dictionary by nearest canonical name
        # For simplicity we’ll just dump max prob and raw class id here
        features_df["Pred_ClassID"] = pred_idx
